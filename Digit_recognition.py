...
def drawResults():
    ...
    for i in range(len(results) * 2 + 2):
        if i > 2 and i % 2 == 1:
            ...
            r1list[j] = rescan.create_rectangle(
                rowW // 2,
                i * rowH,
                rowW // 2 + (rowW * results[j]) // 1,
                (i + 1) * rowH,
                fill="#2727ff",
            )
            r2list[j] = rescan.create_text(
                rowW // 2 - 10,
                ((i + 1 / 2) * rowH) // 1,
                text=str(j),
                anchor="e",
            ...
            r3list[j] = rescan.create_text(
                rowW // 2 + 10,
                ((i + 1 / 2) * rowH) // 1,
                text=str(int(results[j] * 100)) + "%",
                anchor="w",
                fill=globals.white,
            )
            j += 1
